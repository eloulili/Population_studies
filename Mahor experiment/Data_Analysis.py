...
entropy_area_distribution = pd.read_csv(
    "..\AGG_CSV\AGG_CSV\entropy_area_values.csv", sep=","
)
entropy_radius_distribution = pd.read_csv(
    "..\AGG_CSV\AGG_CSV\entropy_radius_values.csv", sep=","
)
...
initial_time = datetime.datetime.strptime(
    np.min(first_time_stamp["Time_Stamp"]), "%Y%m%d-%H%M%S"
)
...
for n_sim in range(1, 69):
    ...
    entropy_radius_values = entropy_radius_distribution[
        entropy_radius_distribution["n_simulations"] == n_sim
    ]["Entropy"]
    n_cells_values = entropy_area_distribution[
        entropy_area_distribution["n_simulations"] == n_sim
    ]["n_cells"]
...
def get_expected_entopy_ratio(n_1, n_2, mean_var):
    return np.log(2 * np.pi * (n_2 + n_1) * mean_var) / np.log(
        2 * np.pi * n_2 * mean_var
    )
...
for trait_name in ["area", "C1_mean", "mean_F_C2", "mean_F_C3"]:
    ...
    if trait_name == "area":
        ...
        n_1_from_ratio_on_mean_variance = get_n1(
            np.log2(np.mean(end_n_cells)),
            mean_cv,
            Total_variance / np.mean(Variance_per_simulation),
        )
        # Using the mean of the ratios
        n_1_from_mean_of_ratios = get_n1(
            np.log2(np.mean(end_n_cells)), mean_cv, np.mean(Ratios)
        )
        ...
        for i in range(len(end_n_cells)):
            ...
            expected_ratios.append(
                1 + ((np.log2(end_n_cells[i]) - 1) / n_1) * (1 + CV[i] ** 2)
            )
        ...
        global_expected_ratio = 1 + ((np.log2(np.mean(end_n_cells)) - 1) / n_1) * (
            1 + mean_cv**2
        )
        ...
        plt.axvline(
            x=mean_ratio, color="r", linestyle="dashed", linewidth=2, label="mean_ratio"
        )
        plt.axvline(
            x=mean_expected_ratio,
            color="b",
            linestyle="dashed",
            linewidth=2,
    ...
    n_1_from_mean_of_ratios_entropy = get_n1(
        np.log2(np.mean(end_n_cells)), mean_cv, np.mean(Ratios_entropy)
    )
    ...
    for i in range(len(end_n_cells)):
        ...
        expected_ratios_entropy.append(
            get_expected_entopy_ratio(n_1, np.log2(end_n_cells[i]), Mean_variance)
        )
    mean_expected_ratio = np.mean(expected_ratios)
    global_expected_ratio = 1 + ((np.log2(np.mean(end_n_cells)) - 1) / n_1) * (
        1 + mean_cv**2
