...
def compute_new_phenotype(
    mother_cell, stds: list[float], probability_to_adapt: float
) -> list[float]:
    """
...
class EvolutiveCell:
    ...
    def reproduce(self, probability_to_adapt: float = 0.1, stds=list[float]):
...
def Gillespie_function(
    ...
    mean_phenotype = [
        [sample.sum_evolution[i] / sample.n for i in range(len(sample.sum_evolution))]
    ]
    current_time = 0
    timesteps = [0]
    n_timesteps = 1
    ...
    while sample.n < max_population_size:
        ...
        next_time = (
            compute_next_time_step(sample.n, base_growth_rate, "lognormal") / sample.n
        )
        ...
        mean_phenotype.append(
            [
                sample.sum_evolution[i] / sample.n
                for i in range(len(sample.sum_evolution))
            ]
        )
...
def main(
    ...
    cells = []
    cells = [
        EvolutiveCell(0, phenotype=first_evolution[i])
    ...
    print(
        f"Mean variance of deviation: {np.mean(variance_deviation_per_simulation, axis=0)}"
    )
    print(f"Mean deviation through replicates: {list_deviation_replicates}")
    print(
        f"Ratio of mean deviation through replicates with individual variance: {np.mean(variance_deviation_per_simulation, axis=0)/list_deviation_replicates}"
    )
    print(
        f"Mean ratio of mean deviation through replicates: {np.mean(variance_deviation_per_simulation, axis=0)/vars}"
    )
    print(
        f"Mean ratio of mean deviation through replicates: {np.mean(np.mean(variance_deviation_per_simulation, axis=0)/vars)}"
    )
    print(
        f"Ratio of mean deviation through replicates: {np.mean(variance_deviation_per_simulation, axis=0)/list_deviation_replicates}"
    )
    print(
        f"Mean ratio of mean deviation through replicates: {np.mean(np.mean(variance_deviation_per_simulation, axis=0)/list_deviation_replicates)}"
    )
    print(
        f"Parameters: stds =  {stds}, adaptation_probability = {adaptation_probability}, growth_rate = {base_growth_rate}, \n"
    )
    return (
        np.mean(
            list_deviation_replicates
            / np.mean(variance_deviation_per_simulation, axis=0)
        ),
        np.mean(vars / np.mean(variance_deviation_per_simulation, axis=0)),
        evolution_first_std,
    )
...
def main_lior(
    ...
    effective_ratio = total_variance / np.mean(
    ...
    print(
        f"Is in the range: {expected_ratio - 1.96*std_ratio<= effective_ratio  and effective_ratio <= expected_ratio + 1.96*std_ratio}"
    )
    ...
    return (
        ...
        CV_2,
        total_entropy,
        np.mean(entropy_per_simulation),
        total_entropy / np.mean(entropy_per_simulation),
    )
