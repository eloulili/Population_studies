...
def gillespie_algorithm(initial_evolutions, initial_populations, total_time, evolution_rate):
    ...
    while time < total_time:
        ...
        if np.random.uniform(0,1) < evol_probability and len(populations) < 10:
            ...
            proportions_history = [np.append(step, -0.1) for step in proportions_history]
...
#np.random.seed(0)
start = time.time()
timesteps, population_history, proportions_history, current_evolutions = gillespie_algorithm( initial_evolutions, initial_populations, total_time, 1000)
stop = time.time()
